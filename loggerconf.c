...
/* Console logger */
static struct { FILE *output; } s_clog;
...
int logger_configure(const char *filename) {
  ...
  if (hasFlag(s_logger, kFileLogger)) {
    if (!logger_initFileLogger(s_flog.filename, s_flog.maxFileSize,
                               s_flog.maxBackupFiles)) {
      return 0;
    }
  }
  if (s_logger == 0) {
...
static void parseLine(char *line) {
  ...
  if (strcmp(key, "level") == 0) {
    ...
    if (strcmp(val, "stdout") == 0) {
      ...
      fprintf(stderr,
              "ERROR: loggerconf: Invalid logger.console.output: `%s`\n", val);
      s_clog.output = NULL;
    }
    ...
    if (nfiles < 0) {
      fprintf(stderr,
              "ERROR: loggerconf: Invalid logger.file.maxBackupFiles: `%s`\n",
              val);
      nfiles = 0;
    }
