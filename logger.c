...
void logger_setLevel(LogLevel level) { s_logLevel = level; }

LogLevel logger_getLevel(void) { return s_logLevel; }

int logger_isEnabled(LogLevel level) { return s_logLevel <= level; }

void logger_autoFlush(long interval) {
  s_flushInterval = interval > 0 ? interval : 0;
}

static int hasFlag(int flags, int flag) { return (flags & flag) == flag; }
...
static void getTimestamp(const struct timeval *time, char *timestamp,
                         size_t size) {
  time_t sec = time->tv_sec; /* a necessary variable to avoid a runtime error on
                                Windows */
  struct tm calendar;
  ...
  len = strftime(timestamp, size, "%Y%m%d %H:%M:%S", &calendar);
  sprintf(&timestamp[len], ".%06ld", (long)time->tv_usec);
}
...
void logger_log(LogLevel level, const char *file, int line, const char *fmt,
  ...
  if (hasFlag(s_logger, kFileLogger)) {
    if (rotateLogFiles()) {
      va_start(farg, fmt);
      s_flog.currentFileSize +=
          vflog(s_flog.output, levelc, timestamp, threadID, file, line, fmt,
